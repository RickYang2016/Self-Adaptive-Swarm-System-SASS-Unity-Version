...
public class Routing : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if(gameObject.GetComponent<AgentState>().agentHP > 10f)
        {
            // if explorers' collision list and Exp-Mon collision list both are equal to zero
            if(gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo.Count == 0 &&
               gameObject.GetComponentInChildren<CollisionDection>().CollisionList.Count == 0)
            {
                ExplorerNoneCollisionPlan();
            }
            else if(gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo.Count != 0 &&
                    gameObject.GetComponentInChildren<CollisionDection>().CollisionList.Count == 0)
            {
                ExpMonCollisionRoutingPlan(gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo);
            }
            else if(gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo.Count == 0 &&
                    gameObject.GetComponentInChildren<CollisionDection>().CollisionList.Count != 0)
            {
                ExplorerCollisionPlan();
            }
            else if(gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo.Count != 0 &&
                    gameObject.GetComponentInChildren<CollisionDection>().CollisionList.Count != 0)
            {
                bool compareDisResult = CompareDistance(gameObject.GetComponentInChildren<CollisionDection>().CollisionList,
                                                     gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo);

                if(compareDisResult == true)
                {
                    ExpMonCollisionRoutingPlan(gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo);
                }
                else
        ...
        else
        {
            ...
            else
            {
                gameObject.transform.localPosition = Vector3.MoveTowards(gameObject.transform.localPosition,
                                                                         gameObject.GetComponent<AgentState>().safetyPos, step);
            }                            
        }
    ...
    private bool CompareDistance(List<CollisionSensorState> CollisionList, List<MonExpColSensorState> agentsInfo)
    {
        bool result = true;
        List<float> explorerDisList = new List<float>();
        List<float> ExpMonDisList = new List<float>();

        foreach(var item in CollisionList)
        {
            explorerDisList.Add(Vector3.Distance(gameObject.GetComponent<AgentState>().agentPos, item.agentPos));
        }

        foreach(var item in agentsInfo)
        {
            ExpMonDisList.Add(Vector3.Distance(gameObject.GetComponent<AgentState>().agentPos, item.agentPos));
        }

        if(explorerDisList.Min() > ExpMonDisList.Min())
        {
            result = true;
        }
        else
    ...
    private void ExplorerNoneCollisionPlan()
    {
        centralDis = Vector3.Distance(gameObject.GetComponent<Formation>().GetCentralPoint(), gameObject.GetComponent<AgentState>().centralGoalPos);

        // whether or not percept monsters
        if(gameObject.GetComponent<CommunicationFramework>().MonstersList.Count == 0 && gameObject.GetComponent<AgentState>().level1Decision == "Patroling")
        {
            if(centralDis > 30 && centralDis < 999)
            {
                // Debug.Log("centralDis: " + centralDis);
                countTimes++;
                gameObject.transform.localPosition = Vector3.MoveTowards(gameObject.transform.localPosition, targetQueue.Dequeue(), step);
            }
            else
            {
                // the last agent get the patroling position, then the group move to the final goal position
                gameObject.transform.localPosition = Vector3.MoveTowards(gameObject.transform.localPosition, gameObject.GetComponent<AgentState>().goalPos, step);
            }
        }
        else
        {
    ...
    private void ExplorerCollisionPlan()
    {
    ...
    private void DeadLockDetect(Dictionary<string, float> sortedCollisionDic)
    {
        ...
        // set sepcifical step to verify deadlock whether or not exit
    ...
    private void CollisionRoutingPlan(Dictionary<string, float> sortedCollisionDic)
    {
        ...
        BoundaryAngle = Math.Abs(Math.Atan(2 * gameObject.GetComponent<AgentState>().agentRadius / 
    ...
    private void ExpMonCollisionRoutingPlan(List<MonExpColSensorState> agentsInfo)
    {
        float stepParameter = 10;
        float l = 0; // central point distance between explorer and nearest monster
        float r = gameObject.GetComponent<AgentState>().agentRadius - 2;
        float R = agentsInfo.First().agentRadius - 3;
        double leftDeflectionAngle = 0;
        double rightDeflectionAngle = 0;
        double deflectionAngle = 30;
        double tmpAngle = 0;
        ...
        // calculate the distance matrix between current agent and monsters in the collision list
        foreach(var item in agentsInfo)
        {
            disMatrix.Add(item.ID, Vector3.Distance(gameObject.GetComponent<AgentState>().agentPos, item.agentPos));
        }

        tmpdisMatrix = monstersSort(disMatrix);

        // avoid the collision to the nearest agent in the collision list

        // get the nearest monster's position
        foreach(var item in agentsInfo)
        {
            if(tmpdisMatrix.Keys.First() == item.ID)
            {
        ...
        l = Vector3.Distance(gameObject.GetComponent<AgentState>().agentPos, nearestAgentPos);

        // calculate the collision boundary between itself and nearest one
        tmpAngle = Math.Abs(Math.Atan((r * Math.Pow((l + R - r), 0.5) / Math.Pow((l - R + r), 0.5) + 
                                   R * Math.Pow((l - R + r), 0.5) / Math.Pow((l + R - r), 0.5)) / (l - R - r)) / Math.PI * 180);

        if(tmpAngle >= 0.000001f)
        {
            BoundaryAngle = tmpAngle;
        }
        else
        {
            BoundaryAngle = 60;
        ...
        tmpDic = GetFinalExpMonDeflectionAngle(leftDeflectionAngle, rightDeflectionAngle);
    ...
    private Dictionary<string, float> monstersSort(Dictionary<string, float> infoDic)
    {
        int tmp;
        Dictionary<int, float> tmpDic = new Dictionary<int, float>();
        Dictionary<string, float> infoDicSort = new Dictionary<string, float>();
    ...
    private Dictionary<string, float> GetFinalExpMonDeflectionAngle(double leftDeflectionAngle, double rightDeflectionAngle)
    {
        int rightSideAgents = 0;
        int leftSideAgents = 0;
        Dictionary<string, float> finalDeflectionAngle = new Dictionary<string, float>();
        ...
        foreach(var item in gameObject.GetComponentInChildren<ExpMonCollisionSensor>().agentsInfo)
