...
public class MonsterRouting : MonoBehaviour
{
    ...
    private void CollisionRoutingPlan(Dictionary<string, float> sortedCollisionDic)
    {
        ...
        BoundaryAngle = Math.Abs(Math.Atan(2 * gameObject.GetComponent<MonsterState>().agentRadius / 
    ...
    // still need to update in this part
    public void MonExpCollisionRoutingPlan(List<ExpMonColSensorState> agentsInfo)
    {
        float stepParameter = 10;
        float l = 0; // central point distance between explorer and nearest monster
        float r = gameObject.GetComponent<MonsterState>().agentRadius - 3;
        float R = agentsInfo.First().agentRadius - 2;
        double leftDeflectionAngle = 0;
        double rightDeflectionAngle = 0;
        double deflectionAngle = 30;
        double tmpAngle = 0f;
        ...
        // calculate the distance matrix between current monster and explorers in the collision list
        ...
        tmpdisMatrix = explorersSort(disMatrix);
        ...
        l = Vector3.Distance(gameObject.GetComponent<MonsterState>().agentPos, nearestAgentPos);

        // calculate the collision boundary between itself and nearest one
        BoundaryAngle = Math.Abs(Math.Atan((r * Math.Pow((l + R - r), 0.5) / Math.Pow((l - R + r), 0.5) + 
        ...
        tmpDic = GetFinalMonExpDeflectionAngle(leftDeflectionAngle, rightDeflectionAngle);
    ...
    private Dictionary<string, float> explorersSort(Dictionary<string, float> infoDic)
    {
        int tmp;
        Dictionary<int, float> tmpDic = new Dictionary<int, float>();
        Dictionary<string, float> infoDicSort = new Dictionary<string, float>();
    ...
    private Dictionary<string, float> GetFinalMonExpDeflectionAngle(double leftDeflectionAngle, double rightDeflectionAngle)
    {
        int rightSideAgents = 0;
        int leftSideAgents = 0;
        Dictionary<string, float> finalDeflectionAngle = new Dictionary<string, float>();
        ...
        foreach(var item in gameObject.GetComponentInChildren<MonExpCollisionSensor>().agentsInfo)
