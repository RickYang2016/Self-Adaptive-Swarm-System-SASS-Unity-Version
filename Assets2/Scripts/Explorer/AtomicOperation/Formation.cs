...
public class Formation : MonoBehaviour
{
    ...
    public void FormationPlan()
    {
        ...
        // numAgents = groupMemberPos.Count;
        ...
        if(gameObject.GetComponent<AgentState>().level1Decision == "Patroling")
        {
            ...
            tasksList.Add(gameObject.GetComponent<AgentState>().goalPos);
        }
        else
        {
            foreach(var item1 in gameObject.GetComponent<Selection>().groupList)
            ...
            // foreach(var item in GetSortedMonstersList(gameObject.GetComponent<CommunicationFramework>().MonstersList,
            //                                           gameObject.GetComponent<AgentState>().level2Decision))
            // {
            //     tasksList.Add(item.Value);
            // }     
        }
        ...
        if(gameObject.GetComponent<AgentState>().level1Decision == "Patroling")
        {
            PatrolFormation(numAgents, player, groupMemberPos, groupNum, tasksList);
        }

        // attacking formation
        else if(gameObject.GetComponent<AgentState>().level1Decision == "Attacking")
        {
            AttackFormation(numAgents, player, groupMemberPos, groupNum, tasksList);
        }

        // defending formation
        else if(gameObject.GetComponent<AgentState>().level1Decision == "Defending")
        {
            DefendFormation(numAgents, player, groupMemberPos, groupNum, tasksList);               
        }
        else
        {
    ...
    public void PatrolFormation(int numAgents, string player, 
                                ...
                                int groupNum, List<Vector3> tasksList)
    {
        int n = numAgents;
        ...
        foreach(var robotLocalPos in groupMemberPos)
        {
            ...
            for(int i = 0; i < Math.Ceiling(n/3.0); i++)
            {
                taskCoordinate = tasksList[tasksList.Count - 1];
        ...
        GetFinalDisPos(robotsTaskDis, robotsTaskPos, tasksList, groupNum);
    }
    ...
    private Dictionary<string, Vector3> GetSortedMonstersList(List<perceptiveMonsterState> MonstersList, string level2Decision)
    {
        ...
        if(level2Decision == "Nearest")
        {
            ...
            foreach(var item1 in  MonstersList)
            {
                foreach(var item2 in gameObject.GetComponent<CommunicationFramework>().CommunicationList)
                {
                    disMatrix.Add(item1.name, Vector3.Distance(item1.agentPos, item2.agentPos));
                }
            }
        ...
        {
            Debug.Log("Level2 decision has something problem!");
        }
    ...
    private Vector3 GetSubGroupPos(Vector3 currentPos, Vector3 taskPos, int numGroups, int groupNum)
    {
        ...
        translationPos = currentPos + new Vector3(gameObject.GetComponent<AgentState>().agentRadius * 20 * (groupNum - 1) / numGroups, 0f, 0f);

        return translationPos;
    }

    // Compute the distance matrix and use the consensus queue to sequentially choose task.
    private void GetFinalDisPos(Dictionary<string, List<float>> robotsTaskDis, Dictionary<string, List<Vector3>> robotsTaskPos, List<Vector3> tasksList, int groupNum)
    {
        ...
        agentFinalPos = GetSubGroupPos(tmpFinalPos, tasksList[tasksList.Count - 1],gameObject.GetComponent<Selection>().numTask, groupNum);
    }
}
